...
class SolverBase:
    def __init__(self, grid):
        self.grid = grid
        self.w, self.h, self.l = np.shape(self.grid)
        self.start_point = (0, 0, 0)
        self.end_point = (self.w - 1, self.h - 1, self.l - 1)
        self.path = []
        self.path_array = []
        self.read_num = 0
        self.write_num = 0
...
class RecursiveDFS(SolverBase):
    ...
    def __move(self, point):
        ...
        result = -1        
        if result == -1:
            result = self.__check_point((x, y, z), (x+1, y, z), DIRECTIONS["E"])
        if result == -1:
            result = self.__check_point((x, y, z), (x-1, y, z), DIRECTIONS["W"])
        if result == -1:
            result = self.__check_point((x, y, z), (x, y+1, z), DIRECTIONS["N"])
        if result == -1:
            result = self.__check_point((x, y, z), (x, y-1, z), DIRECTIONS["S"])
        if result == -1:
            result = self.__check_point((x, y, z), (x, y, z+1), DIRECTIONS["F"])
        if result == -1:
            result = self.__check_point((x, y, z), (x, y, z-1), DIRECTIONS["B"])
